...
- 어려웠던 부분, 오류 상황
  ...
  - 기존 템플렛의 전체 폰트를 원하는 폰트로 수정하려 Bootstrap.css 파일과 Style.css 파일을 변경했으나 일부 기능에서 오류 발생 -> 결국 수정하지 못하고 기존 파일로 회귀함 -> css 파일을 변경할 때는 좀 더 신중을 기해야 한다는 걸 배움, 또한 변경 이전 파일을 Backup 해둬야 한다는 걸 깨달음
  - 달력 API를 이용해 방제력을 구현하고자 했지만 시간 부족과 농업에 대한 이해력 부족으로 구현하지 못 함
